...
int main(int argc, char* argv[])
{
  ...
  double free_energy = spa_free_energy(initial_Hamiltonian, final_temp);
  ...
  filename="spa_mc/spin_arrangement"+current_time_str()+latticedata+".nb"; ofstream outfile_spinarr(filename);
  spinarrangement_2d_Mathematica_output(randsigma,outfile_spinarr);
  filename="spa_mc/results_ising_spa_"+current_time_str()+latticedata+".txt"; ofstream outfile_data(filename);
    ...
    for(double i=9; i>=1; i-= decrement)
    {
      ...
      for(int sweep=0; sweep< N_therm; sweep++)
      {
        for(int lattice_index=0; lattice_index<size*size; lattice_index++)
        {
          ...
          double suggested_free_energy = spa_free_energy(suggested_Hamiltonian, temperature);
      ...
      double final_free_energy = 0; double S_pi =0.0; double magnetisation=0.0; 
      for(int sweep= N_therm; sweep<no_sweeps; sweep++)
      {
        ...
        final_free_energy += free_energy/size*size; 
